...
namespace TorontoBeachPredictor.Service
{
    static class ParseExtensions
    {
        ...
        public static T? TryParseElement<T>(this XElement xElement, String elementName, Func<String, T> parse) where T : struct => xElement.Element(elementName)?.Value.TryParse(parse);
        public static T? TryParseElement<T>(this XElement xElement, String elementName, TryParseDelegate<T> tryParse) where T : struct => xElement.Element(elementName)?.Value.TryParse(tryParse);

        public static T? TryParseAttribute<T>(this XElement xElement, String elementName, Func<String, T> parse) where T : struct => xElement.Attribute(elementName)?.Value.TryParse(parse);
        public static T? TryParseAttribute<T>(this XElement xElement, String elementName, TryParseDelegate<T> tryParse) where T : struct => xElement.Attribute(elementName)?.Value.TryParse(tryParse);
    }
}
